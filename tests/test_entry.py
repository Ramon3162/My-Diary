...
class TestEntryCase(BaseTestClass):
    ...
    def test_post_entry(self):
        ...
        #Not JSON data
        response = self.app.post('/api/v1/entries/', data=self.entry_contents)
        self.assertEqual(response.status_code, 400)

        #No title
        response = self.app.post('/api/v1/entries', data=json.dumps(self.entry_no_title), headers={'content-type':'application/json'})
        self.assertEqual(response.status_code, 400)
        data = json.loads(response.get_data())
        self.assertEqual(data['message'], 'Title should be a string')

        #No description
        response = self.app.post('/api/v1/entries', data=json.dumps(self.entry_no_description), headers={'content-type':'application/json'})
        self.assertEqual(response.status_code, 400)
        data = json.loads(response.get_data())
        self.assertEqual(data['message'], 'Description should be a string')

        #The correct request
        response = self.app.post('/api/v1/entries', dara=json.dumps(self.entry_contents, headers={'content-type':'application/json'}))
    ...
    def test_update_entry(self):
        ...
        response = self.app.put('/api/v1/entries/1', data=json.dumps(dict(title="New Title")), headers={'content-type':'application/json'})
        self.assertEqual(response.status_code, 200)
        data = json.loads(response.get_data())
