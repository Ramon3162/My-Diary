...
class User(Database):
        ...
        if not username_data:
            if not email_data:
                self.cursor.execute("""INSERT INTO diary_users (username, password, email, status)
                                    VALUES (%(username)s, %(password)s, %(email)s, %(status)s)""",
                                    user_data)
                self.conn.commit()
                self.cursor.execute("SELECT * FROM diary_users WHERE username = %s", (username, ))
                data = self.cursor.fetchone()
                return jsonify({'User': User().display_user(data),
    ...
    def update_user_data(self, user_id, username, email, status):
        ...
        if data:
            ...
            return jsonify({'User' : User().display_user(updated_data),
                            'message' : 'User data updated successfully'}), 200
        return jsonify({'message' : 'User not found'})


    def display_user(self, user):
        """Dictionary to hold entry data"""
        self.cursor.execute("""SELECT COUNT(entry_title) FROM diary_entries WHERE id = %s""",
                            (user[0], ))
...
class Entry(Database):
    ...
    def add_entry(self, current_user, title, description, date_posted):
        ...
        self.cursor.execute("""INSERT INTO diary_entries (id, entry_title, description, date_posted)
                            VALUES (%s, %s, %s, %s)""",
                            (current_user, title, description, date_posted,))
        self.conn.commit()
        self.cursor.execute("""SELECT * FROM diary_entries WHERE id = %s AND 
                            entry_id = (SELECT MAX(entry_id) FROM diary_entries)""",
                            (current_user,))
        data = self.cursor.fetchone()
        return jsonify({'Entry': Entry().display_entry(data),
                        'message' : 'Entry created successfully'}), 201
    def get_one_entry(self, entry_id, current_user):
        ...
        if data:
            return jsonify({'Entry' : Entry().display_entry(data),
                            'message' : 'Entry retrieved successfully'}), 200
    ...
    def get_all_entries(self, current_user):
        ...
        if data:
            return jsonify({'Entries' : [Entry().display_entry(entry) for entry in data],
                            'message' : 'All entries found successfully'})
    ...
    def update_entry(self, entry_id, title, description, current_user):
        ...
        if data:
            self.cursor.execute("""UPDATE diary_entries set entry_title = %s,
                                description = %s WHERE entry_id = %s""",
                                (title, description, entry_id, ))
            self.conn.commit()
            self.cursor.execute("""SELECT * FROM diary_entries WHERE entry_id = %s""", (entry_id, ))
            updated_data = self.cursor.fetchone()
            return jsonify({'Entry' : Entry().display_entry(updated_data),
                            'message' : 'Entry updated successfully'}), 200
        return jsonify({'message' : 'Entry not found'})
